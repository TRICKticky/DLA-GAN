...
def train(dataloader, netG, netD, netC, text_encoder, image_encoder, optimizerG, optimizerD, args, writer, new_NetG=False, new_NetC=False):
    ...
    for step, data in enumerate(dataloader, 0):
        ...
        mask = (captions == 0)
        # mask = get_key_padding_mask(captions)
        ...
        # aug = torch.randint(0, 2, (batch_size, z_dim), dtype=torch.float32).to(device)

        if new_NetG:
            fake, _ = netG(noise, sent_emb, words_embs, caption_lens)
        ...
        pred_real, errD_real = predict_loss(netC, real_features, img_feature_local=real_features_local, text_feature=sent_emb, text_feature_local=words_embs, mask=mask, captions=captions, negtive=False, new_NetC=new_NetC)
        ...
        _, errD_mis = predict_loss(netC, mis_features, img_feature_local=mis_features_local, text_feature=sent_emb, text_feature_local=words_embs, mask=mask, captions=captions, negtive=True, new_NetC=new_NetC)
        ...
        _, errD_fake = predict_loss(netC, fake_features, img_feature_local=fake_features_local, text_feature=sent_emb, text_feature_local=words_embs, mask=mask, captions=captions, negtive=True, new_NetC=new_NetC)

        # MA-GP
        errD_MAGP = MA_GP(imgs, sent_emb, pred_real)
        # errD_MAGP = MA_GP(imgs, words_embs, pred_real)
        ...
        if new_NetC:
            # output = netC(fake_features, fake_features_local, sent_emb, words_embs, mask)
            # output = netC(fake_features, sent_emb, words_embs, mask)
            output = netC(fake_features, sent_emb)
            # _, errG = predict_loss(netC, fake_features, img_feature_local=None, text_feature=sent_emb,
            #                             text_feature_local=words_embs, mask=mask, captions=captions, negtive=True,
            #                             new_NetC=new_NetC)
        else:
            output = netC(fake_features, sent_emb)
        ...
        # real_features = netD(imgs)
        # real_features_space = torch.mean(real_features, dim=1)
        # real_features_feature = nn.AdaptiveMaxPool2d(1)(real_features)
        # fake_features_space = torch.mean(fake_features, dim=1)
        # fake_features_feature = nn.AdaptiveMaxPool2d(1)(fake_features)
        #
        # delta_space = torch.abs(real_features_space - fake_features_space)
        # delta_feature = torch.abs(real_features_feature - fake_features_feature)
        # delta_Gloss = (delta_space.mean() + delta_feature.mean()) / 2.0
        # errG = -output.mean() + 0.4 * delta_Gloss# - sim
    ...
    for step, data in enumerate(dataloader, 0):
        ...
        with torch.no_grad():
            ...
            if truncation==True:
                ...
                noise = torch.randn(batch_size, z_dim).to(device)
            fake_imgs = netG(noise, sent_emb, words_embs, mask=None)
...
def test(dataloader, text_encoder, img_encoder, netG, device, m1, s1, epoch, max_epoch,
    ...
    # fid, IS_mean, IS_std = calculate_fid_is(dataloader, text_encoder, img_encoder, netG, device, m1, s1, epoch,
    #                                         max_epoch, times, z_dim, batch_size, truncation, trunc_rate)
    ...
    for time in range(times):#总共10次，因为dataloader在采样的时候是丢弃了最后一个凑不够一个batch_size的batch，所以采样整个数据集多次以达到公平性（每一次采样的数据有差别）
        for i, data in enumerate(dataloader):
            ...
            with torch.no_grad():
                ...
                # fake_imgs, _ = netG(noise, sent_emb, words_embs, caption_lens)
                #规范化生成图像并将其输入V3网络
                fake = norm(fake_imgs)
                pred = model(fake)[0]
...
# import tensorflow.compat.v1 as tf  无compat包
...
##以下为tf对命令行的初始化，这里并不需要这些参数，但是程序运行train.sh脚本时需要读取这些参数，所以全部初始化为空以保证程序正确运行
tf.app.flags.DEFINE_string('stamp',
                           '',
                           "debug")
tf.app.flags.DEFINE_string('cfg',
                           '',
                           "debug")
tf.app.flags.DEFINE_string('num_workers',
                           '',
                           "debug")
tf.app.flags.DEFINE_string('imsize',
                           '',
                           "debug")
tf.app.flags.DEFINE_string('resume_epoch',
                           '',
                           "debug")
tf.app.flags.DEFINE_string('resume_model_path',
                           '',
                           "debug")
...
def generate_samples(noise, caption, words, mask, model):
    #model为netG
    with torch.no_grad():
        # fake, _ = model(noise, caption.cuda(), words.cuda(), mask.cuda())
...
#直接进行语义对齐，收敛更快
def predict_loss(predictor, img_feature, img_feature_local, text_feature, text_feature_local, mask=None, captions=None, negtive=None, new_NetC=False):
    if new_NetC == True:
        # output = predictor(img_feature, img_feature_local, text_feature, text_feature_local, mask)
        output = predictor(img_feature, text_feature)

        # output = predictor.predictor(output)
        # # print(output.size())
        # num_words = text_feature_local.size(2)
        # if captions.size(1) > num_words:
        #     captions = captions[:, :num_words]
        # # print(captions.size())
        # captions = captions.squeeze().cuda()
        # # target_y = torch.zeros((output.shape[0], 1), dtype=torch.long).cuda()
        # # target_y = torch.cat((captions, target_y), 1)
        # criteria = nn.CrossEntropyLoss()
        # target_y = torch.zeros(output.size(), dtype=torch.float32).cuda()
        # #查找词库并标注对应单词的概率为1
        # for i, cap in enumerate(captions):
        #     for j, l in enumerate(cap):
        #         target_y[i][j][l] = 1.0
        # #将每个句子的结尾标志为end，即指向词典第一个索引0，概率为1(bos，eos以及padding均指向词库第一个单词end)
        # target_y[:,-1,0] = 1.0
        # # print(target_y.dtype)
        # # print(target_y)
