...
def get_one_batch_data(dataloader, text_encoder, args):
    ...
    return imgs, sent_emb, words_embs, mask, caption_lens, captions
...
def get_fix_data(train_dl, test_dl, text_encoder, args):
    fixed_image_train, fixed_sent_train, fixed_word_train, fixed_mask_train, fixed_caption_lens_train, fixed_captions_train = get_one_batch_data(train_dl, text_encoder, args)
    fixed_image_test, fixed_sent_test, fixed_word_test, fixed_mask_test, fixed_caption_lens_test, _ = get_one_batch_data(test_dl, text_encoder, args)
    ...
    fixed_caption_lens = torch.cat((fixed_caption_lens_train, fixed_caption_lens_test), dim=0)
    ...
    # print(fixed_sent.dtype)
    # fixed_aug = torch.randint(0, 2, (fixed_image.size(0), args.z_dim), dtype=torch.float32).to(args.device)
    # # print(fixed_noise)
    # aug = fixed_aug.cpu().numpy()
    # np.savetxt('aug.txt', aug, fmt="%d")

    return fixed_image, fixed_sent, fixed_noise, fixed_words, fixed_mask, fixed_caption_lens, fixed_captions_train
