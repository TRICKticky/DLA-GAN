...
def main(args):
    ...
    fixed_img, fixed_sent, fixed_z, fixed_words, fixed_mask, fixed_caption_lens, fixed_captions_train = get_fix_data(train_dl, valid_dl, text_encoder, args)
    ...
    for epoch in range(strat_epoch, args.max_epoch, 1):
        ...
        if epoch%gen_interval==0:
            ...
            sample(fixed_z, fixed_sent, fixed_words, fixed_caption_lens, netG, args.multi_gpus, epoch, args.img_save_dir, writer)
        # end epoch
        ...
        if epoch%test_interval==0:
            ...
            fid = test(valid_dl, text_encoder, image_encoder, netG, args.device, m1, s1, epoch,
                                        args.max_epoch, args.sample_times, args.z_dim, args.batch_size, args.truncation,
                                        args.trunc_rate)
        ...
        # # 输出fixed样本在local_affine中的权重信息
        # if epoch%100==0:
        #     _, weight = netG(fixed_z[0].unsqueeze(0).cuda(), fixed_sent[0].unsqueeze(0).cuda(), fixed_words[0].unsqueeze(0).cuda(), fixed_caption_lens[0].unsqueeze(0).cuda())
        #     #转换成英文
        #     the_caption = []
        #     the_caption_idx = fixed_captions_train[0]
        #     # print(the_caption_idx)
        #     for i in range(fixed_caption_lens[0]):
        #         the_caption.append(train_ds.ixtoword[int(the_caption_idx[i])])
        #     # print(the_caption)
        #     #制作采样单词词典
        #
        #     for l in range(6):
        #         for m in range(2):
        #             words_weight_dict = {}
        #             show_weight = weight[l][m]
        #             for i, word in enumerate(the_caption):
        #                 if word not in words_weight_dict.keys():
        #                     words_weight_dict[word] = show_weight[0][i][0]
        #                 else:
        #                     word = word + str(i)
        #                     words_weight_dict[word] = show_weight[0][i][0]
        #
        #             writer.add_scalars(f'words_affine_{l}_{m}', words_weight_dict, epoch)
        #             # print(words_weight_dict)
        #     # for l in range(3):
        #     #     for m in range(2):
        #     #         for n in range(2):
        #     #             words_weight_dict = {}
        #     #             show_weight = weight[l][m][n]
        #     #             for i, word in enumerate(the_caption):
        #     #                 if word not in words_weight_dict.keys():
        #     #                     words_weight_dict[word] = show_weight[0][i][0]
        #     #                 else:
        ...
        #     #                     words_weight_dict[word] = show_weight[0][i][0]
        #     #
        #     #             writer.add_scalars(f'words_affine_{l}_{m}_{n}', words_weight_dict, epoch)
        #     #             # print(words_weight_dict)
